...
const aboutString = 'Welcome to Halal Checker!' + 
...
const Parser = (res) => {
    ...
    try {
        ...
        const getCompany = root.querySelector('.search-result-data>.search-company>img')?.getAttribute('title')
        const getDates = root.querySelector('.search-result-data>.search-date-expired')
                ...
                Bot.onReplyToMessage(sentMessage.chat.id, sentMessage.message_id, (reply) => {
                    ...
                    .then((res) => {
                        ...
                        if(result !== -1)
                        {
                            ...
                            const createString = `Searching for ${reply.text.toUpperCase()}` + 
                            ...
                            const createString = `Cannot find the result for ${reply.text.toUpperCase()} in JAKIM's database` +
